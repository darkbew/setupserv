#!/usr/bin/env bash
# ==============================================================================
# Mitseri Platform — Deploy User & Identity Setup
# ==============================================================================
#
# Description:
...
#   strict permissions (700/600), and prepares the Docker group membership.
#
# Constraints:
...
create_deploy_user() {
    ...
    if [[ "${DRY_RUN}" == "true" ]]; then
        log_dry "Would create user: ${deploy_user} (shell: /bin/bash, home: /home/${deploy_user})"
        return 0
    fi

    local bash_shell
    bash_shell=$(command -v bash || echo "/bin/bash")

    log_info "Creating operational user '${deploy_user}'..."
    useradd \
        --create-home \
        --shell "${bash_shell}" \
        --comment "Mitseri Platform Deploy User" \
        "${deploy_user}"
...
configure_sudoers() {
    local deploy_user="${DEPLOY_USER:-deploy}"
    local sudoers_file="/etc/sudoers.d/90-mitseri-deploy"
...
# ==============================================================================
# Mitseri Platform — Sudoers Configuration
# ==============================================================================
...
SUDOERS_EOF
    ...
    # Validate sudoers content syntax using visudo before writing
    local tmp_sudoers="/tmp/sudoers_test_$$"
    printf '%s' "${content}" > "${tmp_sudoers}"
...
verify_user_setup() {
    ...
    printf '  Sudoers File:    /etc/sudoers.d/90-mitseri-deploy (440)\n'
